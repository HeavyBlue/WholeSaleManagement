...
BEGIN
	...
	ELSE 
		RAISE EXCEPTION 'Orders quantity must be greater than stock quantity. Orders not be created.';
    END IF;
	RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
FOR EACH ROW EXECUTE FUNCTION 	update_item_quantities();
...
FOR EACH ROW EXECUTE FUNCTION 	create_payment_entries();
...
FOR EACH ROW EXECUTE FUNCTION 	check_and_insert_defaulter();

--Function: calculate_monthly_profit()
CREATE OR REPLACE FUNCTION calculate_monthly_profit()
RETURNS TABLE(year NUMERIC, month NUMERIC, total_sales NUMERIC, total_purchases BIGINT, profit NUMERIC) AS
$$
BEGIN
    RETURN QUERY
    ...
    MonthlyPurchases AS (
        SELECT 
            EXTRACT(YEAR FROM i.In_Date) 	AS year,
...
RETURNS TABLE(customer_id INTEGER, order_id INTEGER, first_name VARCHAR(100), second_name VARCHAR(100), address VARCHAR(200),
phone_number VARCHAR(50), email VARCHAR(50), Image BYTEA) AS
$$
BEGIN
    RETURN QUERY
	SELECT 	c.customer_id, o.order_id, c.first_name, c.second_name, c.address, c.phone_number, c.email, c.Image
	FROM Customer AS c, Defaulter AS d, Payment AS p, Orders AS o
...
BEFORE INSERT ON 				inbound_items
FOR EACH ROW EXECUTE FUNCTION 	add_stock();

--Function: calculate_most_profitable_item
CREATE OR REPLACE FUNCTION calculate_most_profitable_item()
RETURNS TABLE("item id" INTEGER, income NUMERIC, outcome NUMERIC, net NUMERIC, "net per item" NUMERIC) AS $$
BEGIN
	RETURN QUERY
		...
		ROUND(SUM((p.paid_amount + p.pending_amount) - (o.quantity * a.average_price)) / SUM(o.quantity)) AS Net_Per_Item
		FROM payment AS p,
		     orders AS o,
		     (SELECT item_id, ROUND(AVG(price), 2) AS average_price FROM suppliers_item GROUP BY item_id) AS a
		WHERE p.orders_id = o.order_id AND a.item_id = o.item_id 
		GROUP BY o.item_id
